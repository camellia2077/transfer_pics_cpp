...
#include <thread>       // <--- Added for std::thread::hardware_concurrency (optional but useful)
#include <functional>   // <--- Added for std::cref
...
path setupImageOutputSubdirectory(const path& baseOutputDir, const path& imagePath, const string& suffix) {
    ...
    try {
        if (filesystem::create_directories(outputSubDirPath)) {
            // Slightly reduce chattiness, maybe only log on creation?
...
// Note: This function writes to cerr/cout. In a multi-threaded context,
// output from different threads might interleave. For cleaner output,
// consider returning status/errors and logging only from the main thread.
// However, for simplicity, we leave the cout/cerr calls here for now.
...
// This function is now designed to be potentially called from multiple threads.
// Be mindful of shared resources if further modifications are made.
// Currently, fontInfo and fontBuffer are read-only here. Config is read-only.
// Output to cout/cerr might be interleaved if called concurrently.
bool processImageFile(const path& imagePath,
                      const path& baseOutputDir, // Output sub-directory base
                      const Config& config,
                      const stbtt_fontinfo& fontInfo, // Pre-loaded font info (passed by const ref)
                      const vector<unsigned char>& fontBuffer) // Font data buffer (passed by const ref)
{
    // Using stringstream to potentially capture output per-thread later if needed
    // For now, still printing directly but this structure helps if interleaving is bad
    stringstream log_ss;
    log_ss << "\n==================================================" << endl;
    log_ss << "Processing image: " << imagePath.string() << endl;
    log_ss << "==================================================" << endl;

    // Consider locking std::cout if output interleaving becomes problematic
    // std::lock_guard<std::mutex> lock(cout_mutex); // Need to define cout_mutex globally
    cout << log_ss.str(); // Print initial message
    log_ss.str(""); // Clear the stream for further logs
    ...
    if (outputSubDirPath.empty()) {
        cerr << "Skipping image " << imagePath.filename().string() << " due to output directory setup failure." << endl;
        return false;
    }
    ...
    cout << "-> Image " << imagePath.filename().string() << " loaded (" << width << "x" << height << "), Time: "
         << fixed << setprecision(3) << load_duration.count() / 1000.0 << " seconds" << endl;
    if (!imgDataPtr) {
        cerr << "Skipping image " << imagePath.filename().string() << " due to loading failure." << endl;
        return false;
    }
    ...
    // --- Generate ASCII data ---
    cout << "Generating ASCII data for " << imagePath.filename().string() << "..." << endl;
    ...
    cout << "Calculated ASCII grid for " << imagePath.filename().string() << ": " << config.targetWidth << "x" << targetHeight << endl;
    ...
    if (asciiResultData.empty() || asciiResultData[0].empty()) { // Check if data or first row is empty
        cerr << "Error: Failed to generate valid ASCII data for " << imagePath.filename().string() << ". Skipping." << endl;
        return false;
    }
    auto ascii_conv_duration = duration_cast<milliseconds>(ascii_conv_end - ascii_conv_start);
    cout << "-> ASCII data conversion time for " << imagePath.filename().string() << ": " << fixed << setprecision(3) << ascii_conv_duration.count() / 1000.0 << " seconds" << endl;
    ...
    if (outputImageWidthPx <= 0 || outputImageHeightPx <= 0) {
        cerr << "Error: Calculated PNG output image dimensions are invalid (" << outputImageWidthPx << "x" << outputImageHeightPx << ") for " << imagePath.filename().string() << ". Skipping image." << endl;
        return false;
    }
     cout << "Calculated PNG output size for " << imagePath.filename().string() << ": " << outputImageWidthPx << "x" << outputImageHeightPx << endl;


    // --- Process each selected scheme ---
    cout << "Processing color schemes for " << imagePath.filename().string() << ":" << endl;
    ...
    for (const auto& currentScheme : config.schemesToGenerate) {
        ...
        cout << "  Processing scheme: " << schemeSuffix.substr(1)
             << " for " << imagePath.filename().string()
             << ", Outputting PNG to: " << finalPngOutputPath.filename().string() << endl; // Shorter path in log
        ...
        try {
            size_t required_size = static_cast<size_t>(outputImageWidthPx) * outputImageHeightPx * OUTPUT_CHANNELS;
             ...
             // Check against vector's max_size() to prevent overflow before allocation attempt
             if (static_cast<double>(outputImageWidthPx) * outputImageHeightPx > static_cast<double>(outputImageData.max_size() / OUTPUT_CHANNELS) ) {
            ...
            cerr << "Error: Failed to allocate PNG output buffer (Size: " << outputImageWidthPx << "x" << outputImageHeightPx << ") for " << imagePath.filename().string() << "/" << schemeSuffix.substr(1) << ". Out of memory? Skipping scheme." << endl;
            allSchemesSuccessful = false;
            continue; // Skip this scheme
        } catch (const std::exception& e) {
            cerr << "Error: Exception occurred while preparing PNG buffer for " << imagePath.filename().string() << "/" << schemeSuffix.substr(1) << ": " << e.what() << ". Skipping scheme." << endl;
            allSchemesSuccessful = false;
            continue; // Skip this scheme
        }
        ...
        cout << "    -> PNG Rendering time (" << schemeSuffix.substr(1) << "): " << fixed << setprecision(3) << render_png_duration.count() / 1000.0 << " seconds" << endl;
        ...
        if (!saveImage(finalPngOutputPath, outputImageWidthPx, outputImageHeightPx, OUTPUT_CHANNELS, outputImageData)) {
            cerr << "  Error: Failed to save PNG for " << imagePath.filename().string() << "/" << schemeSuffix.substr(1) << "." << endl;
            ...
            cout << "    -> PNG Saving time (" << schemeSuffix.substr(1) << "): " << fixed << setprecision(3) << save_png_duration.count() / 1000.0 << " seconds" << endl;
            // cout << "  Scheme PNG " << schemeSuffix.substr(1) << " saved successfully for " << imagePath.filename().string() << "." << endl; // Reduce chattiness
        }
        ...
        if (config.generatePdfOutput) { // <---- 在这里添加了检查
            ...
            cout << "  Outputting PDF (" << schemeSuffix.substr(1) << ") to: " << finalPdfOutputPath.filename().string() << endl;
            ...
            if (!savePdf(finalPdfOutputPath, asciiResultData, config, currentScheme, bgColor, fgColor)) {
                 cerr << "  Error: Failed to save PDF for " << imagePath.filename().string() << "/" << schemeSuffix.substr(1) << "." << endl;
                 ...
                 cout << "    -> PDF Saving time (" << schemeSuffix.substr(1) << "): " << fixed << setprecision(3) << save_pdf_duration.count() / 1000.0 << " seconds" << endl;
                 // cout << "  Scheme PDF " << schemeSuffix.substr(1) << " saved successfully for " << imagePath.filename().string() << "." << endl; // Reduce chattiness
            }
        } else {
             // Only log this once per image perhaps? Or not at all.
             // cout << "  PDF output is disabled by internal configuration for " << imagePath.filename().string() << "." << endl;
        }
...
int main(int argc, char* argv[]) {
    ...
    // --- Load Font (once) ---
    // These are read-only after this point and can be shared safely by threads
    ...
    try {
        ...
        if (filesystem::is_regular_file(inputPath)) {
            // --- Process Single File (No threading needed here) ---
            cout << "\nInput is a single file. Processing sequentially..." << endl;
            if (isImageFile(inputPath)) {
                path baseOutputDir = inputPath.parent_path(); // Output to image's directory
                // Pass fontBuffer by const reference
            ...
            // --- Process Directory (Using Multi-threading) ---
            cout << "\nInput is a directory. Processing image files concurrently..." << endl;
           ...
           path batchBaseOutputDir = inputPath.parent_path() / batchDirName;
            ...
            if (imageFilesToProcess.empty()) {
                 ...
                 vector<std::future<bool>> futures; // Store futures for async tasks
                 futures.reserve(imageFilesToProcess.size());

                 cout << "Launching tasks..." << endl;
                 // Limit concurrency slightly? (Optional)
                 // unsigned int max_threads = std::thread::hardware_concurrency();
                 // cout << "Using up to " << max_threads << " concurrent threads (if available)." << endl;

                 for(const auto& imgPath : imageFilesToProcess) {
                     // Launch processImageFile asynchronously.
                     // std::launch::async policy requests a new thread if possible.
                     // Pass shared read-only data by const reference using std::cref.
                     // Other arguments (paths) are likely fine by value/const ref implicitly.
                     futures.push_back(
                         std::async(std::launch::async,
                                    processImageFile,
                                    imgPath,                    // Pass path by value/copy
                                    batchBaseOutputDir,         // Pass path by value/copy
                                    std::cref(config),          // Pass const config by reference wrapper
                                    std::cref(fontInfo),        // Pass const fontInfo by reference wrapper
                                    std::cref(fontBuffer))      // Pass const fontBuffer by reference wrapper
                     );
                 }
                 ...
                 for (size_t i = 0; i < futures.size(); ++i) {
                    try {
                        bool success = futures[i].get(); // Wait for task i and get its bool result
                        if (success) {
                            processedCount++;
                        } else {
                            failedCount++;
                        ...
                        // Catch exceptions thrown from within processImageFile
                        cerr << "Error: Exception caught while processing image '"
                             << imageFilesToProcess[i].filename().string() << "': " << e.what() << endl;
                        failedCount++;
                    } catch (...) {
                        cerr << "Error: Unknown exception caught while processing image '"
                             << imageFilesToProcess[i].filename().string() << "'." << endl;
                        failedCount++;
                    }
                 }
            }
    ...
    // If the program exits immediately, uncomment the next two lines:
    // cin.clear(); // Clear potential error flags
    // cin.ignore(numeric_limits<streamsize>::max(), '\n'); // Discard existing input
