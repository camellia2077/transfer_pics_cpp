...
bool writeConfigToFile(const Config& config, const std::filesystem::path& outputFilePath) {
    ...
    configFile << "fontFilename = " << config.fontFilename << "  # Relative path in config.ini" << std::endl;
    configFile << "finalFontPath = " << config.finalFontPath << "  # Resolved absolute path used" << std::endl;
    ...
    // configFile << "baseOutputFilename = " << config.baseOutputFilename << " # (Not directly used for naming scheme)" << std::endl;
    ...
    if (config.schemesToGenerate.empty()) {
        configFile << " # (None specified or loaded, defaults might have been used if applicable elsewhere)" << std::endl;
    } else {
        for (size_t i = 0; i < config.schemesToGenerate.size(); ++i) {
            configFile << colorSchemeToString(config.schemesToGenerate[i]); // 使用现有函数转换枚举为字符串
...
bool processImageFileForPng(const path& imagePath,
    const path& outputSubDirPath, // <--- 修改：直接接收最终输出子目录
    const Config& config,
    const stbtt_fontinfo& fontInfo,
    const vector<unsigned char>& fontBuffer)
{
cout << "\n==================================================" << endl;
cout << "Processing IMAGE: " << imagePath.string() << endl;
cout << "Output Directory: " << outputSubDirPath.string() << endl; // 显示输出目录
cout << "==================================================" << endl;

// 输出子目录现在由调用者 (main) 创建和传入，这里不再需要创建
if (!filesystem::exists(outputSubDirPath) || !filesystem::is_directory(outputSubDirPath)) {
cerr << "Error: Output subdirectory provided does not exist or is not a directory: " << outputSubDirPath.string() << endl;
cerr << "Skipping image " << imagePath.filename().string() << "." << endl;
return false;
}
...
// 使用传入的 outputSubDirPath
path finalPngOutputPath = outputSubDirPath / pngOutputFilename;

cout << "  Processing scheme: " << colorSchemeToString(currentScheme) << " -> PNG: " << finalPngOutputPath.filename().string() << endl;
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        path finalMainOutputDirPath; // Path to write the config file into

        if (filesystem::is_regular_file(inputPath)) {
            cout << "\nInput is a single file." << endl;
            if (isImageFile(inputPath)) {
                // 1. Determine and create the output subdirectory for this single file
                string imageSubDirSuffixWithWidth = "_" + std::to_string(config.targetWidth) + config.imageOutputSubDirSuffix;
                // Use setupImageOutputSubdirectory to create it and get the path
                finalMainOutputDirPath = setupImageOutputSubdirectory(inputPath.parent_path(), inputPath, imageSubDirSuffixWithWidth);

                if (!finalMainOutputDirPath.empty()) {
                     // 2. Write the config file into this directory BEFORE processing
                    path configOutputPath = finalMainOutputDirPath / "_run_config.txt";
                    if (!writeConfigToFile(config, configOutputPath)) {
                        cerr << "Warning: Failed to write configuration file for this run. Continuing processing..." << endl;
                        // Decide if this should be a fatal error or just a warning
                    }

                    // 3. Process the image, passing the specific output directory
                    // Pass finalMainOutputDirPath (the specific subdir) to the processing function
                    if (processImageFileForPng(inputPath, finalMainOutputDirPath, config, fontInfo, fontBuffer)) {
                        processedCount++;
                    } else {
                        failedCount++;
                    }
            ...
            // 1. Determine and create the main batch output directory
            string batchDirName = inputPath.filename().string() + "_" + std::to_string(config.targetWidth) + config.batchOutputSubDirSuffix + "_PNG";
            finalMainOutputDirPath = inputPath.parent_path() / batchDirName; // This is the main batch dir
             try {
                 if(filesystem::create_directories(finalMainOutputDirPath)) {
                     cout << "Created main batch output directory: " << finalMainOutputDirPath.string() << endl;
                 } else if (!filesystem::is_directory(finalMainOutputDirPath)) {
                      cerr << "Error: Failed to create/access main batch output directory: " << finalMainOutputDirPath.string() << endl;
                      return 1;
                 }
             } catch (const filesystem::filesystem_error& e) {
                 cerr << "Error creating batch output directory " << finalMainOutputDirPath.string() << ": " << e.what() << endl;
                 return 1;
             }

             // 2. Write the config file into the main batch directory BEFORE processing any images
             path configOutputPath = finalMainOutputDirPath / "_run_config.txt";
             if (!writeConfigToFile(config, configOutputPath)) {
                  cerr << "Warning: Failed to write configuration file for this batch run. Continuing processing..." << endl;
                  // Decide if this should be a fatal error or just a warning
             }

             // 3. Find images and process them
            ...
            if (imageFilesToProcess.empty()) {
                ...
                for(const auto& imgPath : imageFilesToProcess) {
                    // For each image in batch, determine its *own* output subdirectory inside the main batch dir
                    string imageSubDirSuffixWithWidth = "_" + std::to_string(config.targetWidth) + config.imageOutputSubDirSuffix;
                    // Use setupImageOutputSubdirectory again, but base it inside finalMainOutputDirPath
                    path imageSpecificOutputDir = setupImageOutputSubdirectory(finalMainOutputDirPath, imgPath, imageSubDirSuffixWithWidth);

                    if (!imageSpecificOutputDir.empty()) {
                         // Launch async task, passing the image's specific output directory
                         futures.push_back(
                             std::async(std::launch::async,
                                        processImageFileForPng,
                                        imgPath,
                                        imageSpecificOutputDir, // Pass the specific sub-directory
                                        std::cref(config),
                                        std::cref(fontInfo),
                                        std::cref(fontBuffer))
                         );
                ...
                for (size_t i = 0; i < futures.size(); ++i) {
                    try {
                         // imageFilesToProcess[i] might not be safe if some failed directory creation above.
                         // Logging the specific error inside the future is better.
                        if (futures[i].valid()) { // Check if future is valid before getting result
                             if (futures[i].get()) {
                                 processedCount++;
                             } else {
                                 failedCount++;
                             }
                        } // If future wasn't valid (e.g., task creation failed), it's already counted in failedCount potentially
                    } catch (const std::exception& e) {
                         // Determine which file caused the error is tricky here without more context passing
                         cerr << "Error retrieving result from processing task: " << e.what() << endl;
                         failedCount++;
                    }
                }
            }
